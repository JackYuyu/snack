...
- (void)reloadDataWithModel:(LZCartModel*)model {
    self.goods = model.goods;
    NSString *urls = [NSString stringWithFormat:@"%@%@",BaseURL,self.goods.pic1];
    NSURL *url = [NSURL URLWithString:urls];
    [self.lzImageView sd_setImageWithURL:url];
//    self.lzImageView.image = model.image;
    self.nameLabel.text = self.goods.name;
    self.priceLabel.text = [NSString stringWithFormat:@" %.2f",self.goods.memberPrice.doubleValue ];
...
-(void)setupMainView {
    ...
    priceLabel.frame = CGRectMake(imageView.right +10, bgView.height - 35, width, 30);
    ...
    nameLabel.frame = CGRectMake(imageBgView.right + 10, 10, bgView.width - imageBgView.right-20, 50);
