...
- (void)nextButtonClicked:(id)sender {
    ...
    NSString* title = @"已领取过红包";
    NSString* rewardStatus = [Userdefaults valueForKey:@"rewardStatus"];
    if ([rewardStatus isEqualToString: @"1"]) {
        ...
        title = @"立即分享";
        [Userdefaults setValue:@"1" forKey:@"rewardStatus"];
...
- (void)shareButtonPress:(id)sender {
    ...
    button.enabled   = NO;
    //TODO 自定义分享方式
    NSURL * shareUrl = [NSURL URLWithString:[NSString stringWithFormat:@"itms-apps://itunes.apple.com/app/id%@",DYAppId]];
               NSString * info = DYAppName;
               UIActivityViewController * viewController = [[UIActivityViewController alloc] initWithActivityItems:@[info,[UIImage imageNamed:@"shareIMG"],shareUrl] applicationActivities:nil];
