...
- (UILabel *)numberLabel{
    if (!_numberLabel) {
        _numberLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, self.imageView.top , 15, 15)];
        _numberLabel.backgroundColor = DYColor(255, 99, 71);;
...
- (void)setNub:(NSInteger)proNub{
    ...
    if (nubText.length < 3) {
        ...
        self.numberLabel.right = self.imageView.right+3;
        [self layoutIfNeeded];
    }
